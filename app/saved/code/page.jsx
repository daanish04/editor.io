...
import { deleteCode, getCodeList } from "@/actions/saveState";
...
const CodesPage = () => {
  ...
  useEffect(() => {
    const fetchCodes = async () => {
      ...
      if (response.success) {
        ...
        toast.error("Error fetching codes. Try again later.");
      }
      setLoading(false);
    };
    fetchCodes();
  ...
  const totalPages = Math.ceil(filteredCodes.length / PAGE_LIMIT);
  ...
  const handleDelete = async (id) => {
    ...
    if (response.success) {
      ...
      toast.error("Error deleting code. Try again later.");
            ...
            <TableBody>
              {paginatedCodes.length > 0 ? (
                paginatedCodes.map((code) => (
                  <TableRow key={code.id} className="hover:bg-gray-900">
                    <TableCell className="text-center">
                      <Link
                        href={`/code?id=${code.id}`}
                    ...
                    <TableCell className="text-center">
                      {new Date(code.updatedAt).toLocaleString("en-GB", {
                        day: "numeric",
                        month: "long",
                        year: "numeric",
                      })}
